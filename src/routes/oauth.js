...
module.exports = (config, logger, attachSocketId, isLoggedIn) => {
  ...
  router.get('/redirect', passport.authenticate('github'), (req, res) => {
    ...
    if (req.session.socketId) {
      io.in(req.session.socketId).emit('authenticate-github', profile) // socketId should be attached from GET /oauth/github
    }
    res.end()
  })
